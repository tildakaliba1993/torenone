...
PROVISIONAL module: tests pin the documented (simplified) SANS 10162-1 formulas with
explicit hand-calcs. The *formulas themselves* require registered-engineer sign-off
(standard PDF absent from `standards/`); these tests guard the implementation.
...
class TestBoltResistances:
    def test_m20_88_tension(self):
        # Tr = 0.75·0.80·As·Fu = 0.6·245·800 = 117 600 N = 117.6 kN
        assert bolt_tension_resistance_kn(make_bolt("M20", "8.8")) == pytest.approx(117.6)

    def test_m20_88_shear(self):
        # Vr = 0.60·0.80·As·Fu = 0.48·245·800 = 94 080 N = 94.08 kN
        assert bolt_shear_resistance_kn(make_bolt("M20", "8.8")) == pytest.approx(94.08)

    def test_m20_bearing_on_20mm_plate(self):
        # Br = 3·0.80·t·d·Fu = 2.4·20·20·470 = 451 200 N = 451.2 kN
        br = bolt_bearing_resistance_kn(make_bolt("M20", "8.8"), 20.0, 470.0)
        assert br == pytest.approx(451.2)
    ...
    def test_grade_1090_is_stronger(self):
        ...
        assert t109 > t88
        # M24 10.9: 0.6·353·1000 = 211 800 N
        assert t109 == pytest.approx(211.8)
...
class TestCheckMomentConnection:
    ...
    def test_combined_is_elliptical(self):
        ...
        assert uc == pytest.approx(math.sqrt(ut**2 + us**2), rel=1e-3)
