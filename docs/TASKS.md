...
>
> **Status:** v1.1 · **Last updated:** 2026-06-11 (1.17 done — concrete pad footings, PROVISIONAL; next: 1.18 integration + tonnage)
...
- [x] **1.17 Pad footings (SANS 10100-1)** — `foundations/pad_footing.py`: `check_pad_footing()` → 4 `CheckResult`s (soil bearing service+self-wt vs **engineer-supplied allowable pressure, never assumed**; punching shear; one-way shear; flexure→reinforcement) and `design_pad_footing()` auto-sizes plan (from bearing) → thickness → reinforcement ladders → `PadFootingDesignResult` (plan/thickness/checks/passed/max_utilisation), strongest-tried if unsatisfiable. ⚠️⚠️ **Concrete = a NEW standard surface (SANS 10100-1) — extra-prominent PROVISIONAL** (BS 8110-lineage simplified formulas; all coefficients flagged in clauses + `SOURCES.md`, concrete-experienced engineer sign-off required). *(The `FrameSpec` allowable-bearing input is added in 1.18 integration.)* **14 tests** (`kernel/tests/test_pad_footing.py`): bearing/punching hand-calcs, allowable-pressure sensitivity, reinforcement sensitivity, undersized fails, auto-design passes, plan≥bearing-demand, weaker-soil→bigger-pad, unsatisfiable→strongest, determinism, steel-area formula. All passing (ruff + mypy clean). Full suite: **662 passed**.
