...
- [ ] **5.6 Supabase-backed `ReportStore`** — replace `InMemoryReportStore` with a Supabase implementation: upload the PDF to the private `reports` Storage bucket under `<firm_id>/…`, write the `runs` + `reports` rows, and wire `/design` persistence. The `ReportStore` Protocol in `service/.../reports.py` is the seam (designed for exactly this swap). *(Application-code half of the Supabase backend — the data model + RLS + bucket from 5.1–5.5 are the foundation it builds on.)*
